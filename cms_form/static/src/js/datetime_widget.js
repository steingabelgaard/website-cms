odoo.define('cms_form.datetime_widget', function (require) {
  'use strict';

  var sAnimation = require('website.content.snippets.animation');
  var time_utils = require('web.time');

  sAnimation.registry.CMSDatetimeWidget = sAnimation.Class.extend({
    selector: ".cms_form_wrapper form input.js_datetimepicker",
    start: function () {
    ...
    load_options: function() {
      ...
      this.picker_options = _.defaults(
        ...
        this.$el.data('params').dp || {},
        {
          ...
          format: time_utils.getLangDatetimeFormat(),
          useCurrent: false
        }
      )
    },
    setup_datetimepicker: function() {
      var self = this;
      ...
      if (!placeholder && !_.isUndefined(placeholder)) {
        ...
        and translate it.
        */
        this.$el.attr('placeholder', time_utils.getLangDatetimeFormat());
      }
      ...
      this.$el.on('dp.change', function(e){
        ...
        if (e.date) {
          real_val = e.date.format('YYYY-MM-DD HH:MM');
        }
      ...
      this.$el
        .closest('.input-group')
        .find('.js_datetimepicker_trigger')
        .click(function () {
          self.picker.show();
        });
    },
